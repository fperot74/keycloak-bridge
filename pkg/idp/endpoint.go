...
type Endpoints struct {
	...
	AddUserAttribute             endpoint.Endpoint
	DeleteUserAttribute          endpoint.Endpoint
...
func NewEndpoints(component Component, endpointUpdater func(endpoint cs.Endpoint, name string) endpoint.Endpoint) Endpoints {
	return Endpoints{
		...
		AddUserAttribute:             endpointUpdater(MakeAddUserAttributeEndpoint(component), "add_user_attribute_endpoint"),
		DeleteUserAttribute:          endpointUpdater(MakeDeleteUserAttributeEndpoint(component), "delete_user_attribute_endpoint"),
...
func MakeGetUsersWithAttributeEndpoint(component Component) cs.Endpoint {
	return func(ctx context.Context, req any) (any, error) {
		...
		// At least one of username or groupName must be provided
		if username == nil && groupName == nil {
			return nil, errorhandler.CreateBadRequestError(fmt.Sprintf("%s.%sor%s", msg.MsgErrMissingParam, prmUsername, prmGroupName))
		}
...
// MakeAddUserAttributeEndpoint creates an endpoint for AddUserAttribute
func MakeAddUserAttributeEndpoint(component Component) cs.Endpoint {
	return func(ctx context.Context, req any) (any, error) {
		var m = req.(map[string]string)
		var attribValue string
		if body, ok := m[reqBody]; ok {
			if err := json.Unmarshal([]byte(body), &attribValue); err != nil {
				return nil, errorhandler.CreateBadRequestError(msg.MsgErrInvalidParam + "." + msg.Body)
			}
		} else {
			return nil, errorhandler.CreateBadRequestError(msg.MsgErrMissingParam + "." + msg.Body)
		}
		return commonhttp.StatusNoContent{}, component.AddUserAttribute(ctx, m[prmRealm], m[prmUser], m[prmAttribKey], attribValue)
	}
}

// MakeDeleteUserAttributeEndpoint creates an endpoint for DeleteUserAttribute
func MakeDeleteUserAttributeEndpoint(component Component) cs.Endpoint {
	return func(ctx context.Context, req any) (any, error) {
		var m = req.(map[string]string)
		return commonhttp.StatusNoContent{}, component.DeleteUserAttribute(ctx, m[prmRealm], m[prmUser], m[prmAttribKey])
	}
}
