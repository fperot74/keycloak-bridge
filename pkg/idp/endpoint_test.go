...
func TestAddUserAttributeEndpoint(t *testing.T) {
	var mocks = createMocks(t)
	defer mocks.finish()
	...
	t.Run("Missing body", func(t *testing.T) {
		_, err := mocks.newEndpoints().AddUserAttribute(ctx, map[string]string{})
		assert.Error(t, err)
	})
	t.Run("Body is not a JSON value for a string", func(t *testing.T) {
		_, err := mocks.newEndpoints().AddUserAttribute(ctx, map[string]string{prmRealm: realm, reqBody: `{}`})
		assert.Error(t, err)
	})
	t.Run("Success", func(t *testing.T) {
		mocks.component.EXPECT().AddUserAttribute(ctx, realm, userID, "key", "attribute-value").Return(nil)
		_, err := mocks.newEndpoints().AddUserAttribute(ctx, map[string]string{prmRealm: realm, prmUser: userID, prmAttribKey: "key", reqBody: `"attribute-value"`})
		assert.NoError(t, err)
	})
}

func TestDeleteUserAttributeEndpoint(t *testing.T) {
	var mocks = createMocks(t)
	defer mocks.finish()
	...
	mocks.component.EXPECT().DeleteUserAttribute(ctx, realm, userID, "key").Return(nil)
	_, err := mocks.newEndpoints().DeleteUserAttribute(ctx, map[string]string{prmRealm: realm, prmUser: userID, prmAttribKey: "key"})
	assert.NoError(t, err)
}
